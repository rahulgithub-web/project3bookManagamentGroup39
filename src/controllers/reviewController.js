  ...
  try {
    ...
    if (!rating) {
      return res.status(400).send({
        status: false,
        message:
          "Rating is required it should not be 0 ",
      });
    }
    if (!(rating>=1 && rating <=5))
    return res 
    .status(400)
    .send({ status: false, msg: " No. should be between 1&5"})
...
const updateReview = async (req, res) => {
  try {
    ...
    let checkReview = await reviewModel.findOne({bookId: bookId, reviewId: reviewId, isDeleted: false});
    if(!checkReview) {
